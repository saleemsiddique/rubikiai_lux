...
type CouponEmailParams = {
  unitAmount: number;
  quantity: number;
  currency?: string;
  codes: { code: string; remaining: number }[];
  expiresAt: string; // ISO (YYYY-MM-DD recomendado)
  /** CID de la imagen del logo adjunta en el email (attachments) */
  logoCid?: string; // por defecto 'rubikiai-logo'
};

export function CouponPurchaseEmailHtmlEN(params: CouponEmailParams): string {
