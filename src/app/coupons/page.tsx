...
export default function GiftCardPage(): JSX.Element {
  ...
  const handleBuy = () => {
    // Mockup de UI: Aquí iría la lógica de pago real (por ejemplo, con Stripe).
    const total = (selected * quantity).toFixed(2);
    // Usamos console.error o un modal custom en lugar de alert()
    console.error(`Acción de compra simulada: Comprar ${quantity} x ${selected}€ (Total: ${total} €)`);

    // Mensaje de confirmación visual (usando un alert simple para el propósito del mockup)
    const message = `¡Estás comprando ${quantity} cupón(es) de ${selected}€!\n\nEsta es una simulación estética. En un entorno real, procederías al pago.`;
    
    // NOTA: En un entorno de producción, usa un modal en lugar de alert().
  ...
  return (
    // Aplicamos las variables CSS en línea para que Tailwind pueda acceder a ellas
    <div className="min-h-screen flex flex-col items-center pt-24 px-4 md:px-8" style={{ '--color-primary': '#bfa58b', '--color-primary-dark': '#8f6e52', '--color-secondary': '#7b8ed6', '--color-background-main': '#f4efe9', '--color-background-soft': '#fafafa', '--color-text': '#0f172a', '--color-highlight': '#214235' } as React.CSSProperties}>
      ...
      <main className="w-full max-w-6xl mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Sección de Selección de Cupones */}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="mt-6 flex flex-col sm:flex-row sm:items-end sm:gap-6 gap-4 border-t pt-6 border-[var(--color-primary)]/20">
              ...
              <div className="flex-1">
                {/* Espacio para alinear el botón correctamente */}
              </div>

              <div className="flex items-end">
                <button
                  onClick={handleBuy}
                  className="px-6 py-3 rounded-xl font-bold text-lg shadow-lg hover:scale-[1.03] transition-transform duration-200"
                  style={{ background: 'var(--color-secondary)', color: 'white' }}
                >
                  Comprar Ahora ({quantity} x {selected}€)
                </button>
              </div>
            </div>
        ...
        <aside className="bg-[var(--color-background-soft)] rounded-2xl p-6 shadow-xl flex flex-col gap-6 border border-[var(--color-secondary)]/40 h-fit sticky top-12">
          ...
          <div className="rounded-xl overflow-hidden relative shadow-lg">
            ...
            <div className="p-6" style={{ background: 'linear-gradient(135deg, var(--color-primary-dark), var(--color-primary))' }}>
              <div className="flex items-start justify-between">
                ...
                {/* Código de Referencia eliminado por solicitud del usuario */}
              </div>
            </div>
          ...
          <div className="mt-2">
            ...
            <ul className="text-sm mt-2 space-y-1 text-[var(--color-text)]/70">
              ...
              <li>• Se envía por email al destinatario (o para imprimir).</li>
              <li>• Caducidad de 12 meses.</li>
            </ul>
          </div>
          
          {/* Botón de Vista Previa eliminado por solicitud del usuario */}
        </aside>
      </main>
